...
# Ball 2
x_s = random.randint(-9,9)
y_s = random.randint(-9,9)

x_loco = int(SCREEN_WIDTH/2)
y_loco = int(SCREEN_WIDTH/2)

ball_size = random.randint(12, 32)
...
while not done:
    ...
    # Ball 2
    pygame.draw.circle (screen, ball_color, [x_loco, y_loco], ball_size)
  
    if x_loco >= SCREEN_WIDTH - ball_size or x_loco < ball_size:
        x_s = x_s * -3

    if y_loco >= SCREEN_HEIGHT - ball_size or y_loco < ball_size:
        y_s = y_s * -3
